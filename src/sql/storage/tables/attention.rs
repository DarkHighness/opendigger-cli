...
impl StorageTable for AttentionTable {
    fn name(&self) -> &str {
        "Attention"
    }
    ...
    pub async fn build(
        ...
        let metric = match r#type {
            crate::api::Type::Repository => {
                crate::api::Metric::Repo(crate::api::RepoMetric::Attention)
            }
            crate::api::Type::User => {
                return Err(StorageBuildError::UnsupoortedType(
                    "AttentionTable".to_string(),
                    r#type,
                ))
            }
        };
