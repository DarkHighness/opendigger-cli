...
注意指标中有一部分本身就是图状数据，例如开发者之间的网络、仓库之间的网络，对于这部分数据，我们考虑使用Cypher查询图形数据
...
使用RUST作为开发语言。选择RUST的理由是，RUST是一种高性能、可靠、安全的系统编程语言，并且具有出色的并发和内存管理能力。该语言现代，安全，且编译产物是单二进制程序，方便分发，适合跨平台，利于交叉编译。

##### **2. ChatGPT支持** 

可以配置OPENAI_API_KEY，使命令行工具支持ChatGPT生成查询语句。使用该技术是为了方便对SQL语言不了解的用户使用该CLI工具。

##### **3. TUI交互** 

TUI界面以文本形式呈现，相比于GUI更轻量级，易于修改和维护。

##### **4. DevOps自动构建**

这个过程包括编译代码、运行测试、构建和部署应用程序，自动化构建可以显著提高应用程序的构建速度和开发效率。
